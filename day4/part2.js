function process(input) {
  ...
  input.forEach(function (line) {
    ...
    if(decryptedName.search(/storage/) != -1) {
      console.log(decryptedName, args.sectorID)
    }
  })
...
function decrypt(name, sectorID) {
  ...
  for(var i = 0; i < name.length; i++) {
    ...
    position += sectorID
    position = position % 26 // Get the position back into the alphabet bounds
    result.push(alphabet.charAt(position))

  }
  return result.join("")
}

module.exports = {process, seperateIntoArguments, frequencySort, generateChecksum, decrypt}
